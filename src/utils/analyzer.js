 ...
 */
async function extractFrames(videoPath, jobDir, numFrames = 5) {
  ...
  for (let i = 1; i <= numFrames; i++) {
    ...
    extractPromises.push(
      new Promise((resolve, reject) => {
        ...
        execFile('ffmpeg', args, { timeout: 45_000 }, (err) => {
          if (err) reject(err);
          else resolve();
        });
      })
  ...
  // Extract frames sequentially (not parallel) to avoid OOM on low-memory servers
  for (let i = 0; i < extractPromises.length; i++) {
    try {
      await extractPromises[i];
    } catch (e) {
      console.warn(`[analyzer] Frame ${i + 1} extraction failed: ${e.message}`);
    }
  }
  return framePaths.filter(p => {
