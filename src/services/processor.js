...
async function processVideo(videoPath, startTime, endTime, jobId, options = {}) {
  ...
  console.log(`[processor] Processing clip: ${startSec}s - ${endSec}s (${duration.toFixed(1)}s)${alreadyTrimmed ? ' [pre-trimmed]' : ''}`);
  ...
  if (forceCenter) {
    ...
    try {
      const analysis = await analyzeVideo(videoPath, jobDir, { numFrames: 5 });
      ...
      console.warn(`[processor] Analysis failed, falling back to center crop: ${e.message}`);
      cropParams = calculateCropParams(dims.width, dims.height, dims.width / 2, targetWidth, targetHeight);
    }
  }

  // Step 3: Cut + crop + scale + encode in a single FFmpeg pass
  // If alreadyTrimmed, skip -ss/-t (the segment was already downloaded by the downloader)
  const ffmpegArgs = [
    ...(alreadyTrimmed ? [] : ['-ss', startSec.toString()]),
    '-i', videoPath,
    ...(alreadyTrimmed ? [] : ['-t', duration.toString()]),
    '-filter_complex',
