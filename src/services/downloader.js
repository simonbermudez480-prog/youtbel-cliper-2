...
async function downloadWithCobalt(url, jobDir, cookies) {
  ...
  // DO NOT put cookies in the body — newer Cobalt rejects it as invalid_body
  // Send cookies as a standard Cookie header instead
  ...
  // Convert cookie objects to standard Cookie header string: "NAME=VALUE; NAME2=VALUE2"
  if (hasCookies) {
    const cookieStr = allCookies.map(c => `${c.name}=${c.value}`).join('; ');
    reqHeaders['Cookie'] = cookieStr;
    console.log(`[downloader] Sending ${allCookies.length} cookies via Cookie header`);
  }

  // Wake up Cobalt if it's sleeping (Render free tier spins down after 15 min)
  // Hit the root endpoint first, wait for it to respond, then make the real request
  try {
    console.log(`[downloader] Waking up Cobalt...`);
    const base = COBALT_URL.replace(/\/$/, '');
    const wakeResp = await request(base, { method: 'GET', timeout: 30_000 });
    console.log(`[downloader] Cobalt wake status: ${wakeResp.status}`);
  } catch (e) {
    console.log(`[downloader] Cobalt wake ping failed (might still be starting): ${e.message}`);
  }

  // Retry up to 3 times on 502 (service still waking up)
  let resp;
  let lastError;
  const maxRetries = 3;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      // 502 = Cobalt still starting, retry with delay
      if (resp.status === 502 && attempt < maxRetries - 1) {
        const delay = retryDelays[attempt];
        console.log(`[downloader] Cobalt 502 (attempt ${attempt + 1}/${maxRetries}), retrying in ${delay / 1000}s...`);
      ...
      if (attempt < maxRetries - 1) {
        const delay = retryDelays[attempt];
        console.log(`[downloader] Cobalt request error (attempt ${attempt + 1}/${maxRetries}): ${e.message}, retrying in ${delay / 1000}s...`);
        await new Promise(r => setTimeout(r, delay));
      }
    }
  }
  ...
  if (!resp) {
    throw new Error(`Cobalt unreachable after ${maxRetries} retries: ${lastError?.message || 'unknown'}`);
  }
