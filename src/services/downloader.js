...
/**
 * Get a stream URL from Piped instances (proxy URLs work from any IP).
 */
async function getPipedStreamUrl(videoId) {
  for (const instance of PIPED_INSTANCES) {
    try {
      ...
      if (data.videoStreams && data.videoStreams.length > 0) {
        ...
        for (const s of sorted) {
          if (!s.quality || parseInt(s.quality) <= 1080) {
            console.log(`[downloader] Piped stream URL from ${instance} (quality: ${s.quality || 'unknown'})`);
            return s.url;
          }
        }
        if (sorted.length > 0) {
          console.log(`[downloader] Piped stream URL from ${instance} (fallback quality)`);
  ...
  throw new Error('No progressive stream found on Piped');
}

/**
 * Get a stream URL from Invidious instances.
 */
async function getInvidiousStreamUrl(videoId) {
  for (const instance of INVIDIOUS_INSTANCES) {
    try {
      const resp = await request(
        `${instance}/api/v1/videos/${videoId}?format=json&fields=formatStreams`,
        { timeout: 15_000 }
      );
      if (resp.status !== 200) continue;
      ...
      if (data.formatStreams && data.formatStreams.length > 0) {
        ...
        if (stream?.url) {
          console.log(`[downloader] Invidious stream URL from ${instance}`);
          return stream.url;
        }
      }
    } catch (e) {
...
async function downloadVideoSegment(url, jobId, startTime, endTime) {
  ...
  console.log(`[downloader] Segment download: ${startTime}s - ${endTime}s (${duration.toFixed(1)}s)`);
  ...
  await new Promise((resolve, reject) => {
    ...
    const proc = execFile('ffmpeg', args, { timeout: 180_000 }, (err, _stdout, stderr) => {
      if (err) {
        reject(new Error(`FFmpeg segment download failed: ${(stderr || '').slice(-300) || err.message}`));
        return;
      }
      resolve();
    });
  ...
  console.log(`[downloader] Segment downloaded: ${(stats.size / (1024 * 1024)).toFixed(1)}MB (vs full video would be ~500MB+)`);
  return outputPath;
}
